...
function mergeRangeTreeChildren(parentTrees) {
    ...
    var openRange;
    while (true) {
        var event_1 = startEventQueue.next();
        if (event_1 === undefined) {
            break;
        }
        if (openRange !== undefined && openRange.end <= event_1.offset) {
            result.push(nextChild(openRange, parentToNested));
            openRange = undefined;
        }
        if (openRange === undefined) {
            var openRangeEnd = event_1.offset + 1;
            for (var _i = 0, _a = event_1.trees; _i < _a.length; _i++) {
                var _b = _a[_i], parentIndex = _b.parentIndex, tree = _b.tree;
                openRangeEnd = Math.max(openRangeEnd, tree.end);
                insertChild(parentToNested, parentIndex, tree);
            }
            startEventQueue.setPendingOffset(openRangeEnd);
            openRange = { start: event_1.offset, end: openRangeEnd };
        }
        else {
            for (var _c = 0, _d = event_1.trees; _c < _d.length; _c++) {
